...
void Connection::Write() {
  if (!out_.Empty()) {
    boost::asio::async_write(
        socket_,
        boost::asio::buffer(out_.Front().data(), out_.Front().length()),
...
void Connection::ReadHeader() {
  boost::asio::async_read_until(
      socket_, boost::asio::dynamic_buffer(unfinished_message_), "\r\n\r\n",
      [this](error_code ec, std::size_t) {
        if (!ec) {
          auto body_size = CalculateBodyLength();
          if (body_size > 0) {
            unfinished_message_.resize(unfinished_message_.length() +
                                       body_size);
            ReadBody(body_size);
          } else {
            AddToIncomingQueue();
...
void Connection::ReadBody(size_t body_length) {
  boost::asio::async_read(
      ...
      boost::asio::buffer(unfinished_message_.data() +
                              unfinished_message_.length() - body_length,
                          body_length),
      [this](error_code ec, std::size_t) {
        if (!ec) {
          AddToIncomingQueue();
...
size_t Connection::CalculateBodyLength() {
  ...
  auto content_length =
      std::stoul(unfinished_message_.substr(content_length_pos + 16));
...
void Connection::AddToIncomingQueue() {
  if (owner_ == SERVER)
    in_.EmplaceBack(
        std::make_pair(shared_from_this(), std::move(unfinished_message_)));
