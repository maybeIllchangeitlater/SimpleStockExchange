...
ViewTransactionsPopup::ViewTransactionsPopup(QWidget *parent)
    : QDialog(parent), ui(new Ui::ViewTransactionsPopup) {
  ui->setupUi(this);
  ...
  ui->TransactionViewer->setHorizontalHeaderLabels(
      {s21::BDNames::joined_buyer_name, s21::BDNames::joined_seller_name,
...
ViewTransactionsPopup::~ViewTransactionsPopup() { delete ui; }

void ViewTransactionsPopup::DisplayTransactions(
    const std::string &raw_response) {
  QJsonArray jsons =
      QJsonDocument::fromJson(QString::fromStdString(raw_response).toUtf8())
          .array();
  for (const auto &json : jsons) {
    int row = ui->TransactionViewer->rowCount();
    ui->TransactionViewer->insertRow(row);
    ui->TransactionViewer->setItem(
        row, 0,
