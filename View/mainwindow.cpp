...
void MainWindow::HandleLoginAttempt(const std::string username,
...
void MainWindow::HandleRegisterAttempt(const std::string username,
                                       const std::string password,
...
void MainWindow::HandleCreateBid(const std::string quantity,
  ...
  if (!s21::ResponseParser::CheckIfTransactionsWereMade(server_response)) {
    ...
    ui->ServerMessageInitScreen->setText(
        QString::fromStdString(s21::ResponseParser::CleanBidCreatedResponse(
            std::move(server_response))));
  } else {
    server_response = newtrans_pop_->DisplayNewTransactions(server_response);
    ui->ServerMessageInitScreen->setText(QString::fromStdString(
        server_response.empty() ? s21::ClientDisplayMessages::bid_fulfilled
                                : server_response));
    if (view_bid_pop_->isVisible()) {
      view_bid_pop_->InsertNewBid(server_response);
    }
...
void MainWindow::HandleViewBid(const std::string bid_type) {
  if (bid_type.find(s21::ClientDisplayMessages::sell_bid_type) !=
      std::string::npos) {
    ...
    auto server_response = client_.Inbox().PopFront().second;
    view_bid_pop_->ShowBids(
        s21::ResponseParser::CleanServerResponse(server_response),
        s21::ClientDisplayMessages::sell_bid_type);
  } else if (bid_type.find(s21::ClientDisplayMessages::buy_bid_type) !=
             std::string::npos) {
    ...
    ui->ServerMessageInitScreen->setText(
        s21::ClientDisplayMessages::invalid_bid_type);
  }
  create_bid_pop_->hide();
  create_bid_pop_->close();
}

void MainWindow::HandleUpdateBid(const std::string bid_id,
                                 const std::string bid_rate,
  ...
  std::string detailed_bid =
      s21::ResponseParser::ExtractDetailedBidInfoRaw(server_response);
  std::string display_msg =
      detailed_bid.empty()
          ? ""
          : s21::ResponseParser::ExtractCleanBidUpdateResponse(detailed_bid);
  if (!s21::ResponseParser::CheckIfTransactionsWereMade(server_response)) {
    ui->ServerMessageInitScreen->setText(QString::fromStdString(display_msg));
    if (view_bid_pop_->isVisible()) {
    ...
    ui->ServerMessageInitScreen->setText(QString::fromStdString(
        display_msg.empty() ? s21::ClientDisplayMessages::bid_fulfilled
                            : display_msg));
    if (view_bid_pop_->isVisible()) {
      view_bid_pop_->InsertUpdatedBidBack(detailed_bid, index);
    }
...
void MainWindow::ConnectToHandlers() {
  connect(reg_pop_.get(), &RegisterPopup::RegisterAttempt, this,
          &MainWindow::HandleRegisterAttempt);
  connect(log_pop_.get(), &LoginPopup::LoginAttempt, this,
          &MainWindow::HandleLoginAttempt);
  connect(create_bid_pop_.get(), &CreateBidPopup::MakeBid, this,
          &MainWindow::HandleCreateBid);
  connect(upd_bid_pop_.get(), &UpdateBidPopup::UpdateBid, this,
          &MainWindow::HandleUpdateBid);
  connect(view_bid_pop_.get(), &ViewBids::ViewBid, this,
          &MainWindow::HandleViewBid);
  connect(quot_pop_.get(), &QuotationsPopup::Quotations, this,
          &MainWindow::HandleViewQuotations);

  connect(ui->Logout, &QPushButton::clicked, this, [&]() {
    client_.Disconnect();
    ui->ServerMessageInitScreen->setText(
        s21::ClientDisplayMessages::logged_out);
    ui->LoggedAs->clear();
    SetNotLoginnedButtons();
  });

  connect(view_bid_pop_.get(), &ViewBids::CancelBid, this,
          [&](const std::string bid_id) {
            ...
            ui->ServerMessageInitScreen->setText(
                error_msg.empty() ? s21::ClientDisplayMessages::bid_cancelled
                                  : QString::fromStdString(error_msg));
          });
  connect(view_bid_pop_.get(), &ViewBids::UpdateBid, this,
          [&](const std::string bid_id, const std::string bid_rate,
              const std::string bid_quantity, int bid_index) {
            upd_bid_pop_->SetParameters(bid_id, bid_rate, bid_quantity,
                                        bid_index);
            upd_bid_pop_->exec();
          });
  connect(
      viewtrans_pop_.get(), &ViewTransactionsPopup::ShowBuyTransactions, this,
      [&]() {
        ...
        if (!s21::ResponseParser::CheckStatus(server_response)) {
          ...
          viewtrans_pop_->DisplayTransactions(
              s21::ResponseParser::ExtractTransactionInfoJson(server_response));
        }
      });
  connect(
      viewtrans_pop_.get(), &ViewTransactionsPopup::ShowSellTransactions, this,
      [&]() {
...
void MainWindow::ConnectToSettingsHandlers() {
  connect(
      user_settings_.get(), &UserSettings::DeleteAccount, this,
      [&](const std::string user_password) {
        if (client_.DeleteMe(user_password)) {
          ui->LoggedAs->clear();
          ui->ServerMessageInitScreen->setText(
              s21::ClientDisplayMessages::account_del);
          SetNotLoginnedButtons();
  ...
  connect(
      user_settings_.get(), &UserSettings::ChangeUsername, this,
      [&](const std::string user_name) {
        if (client_.ChangeName(user_name)) {
          ...
          user_settings_->SetStatus(s21::ClientDisplayMessages::namechange +
                                    user_name);
          ui->ServerMessageInitScreen->setText(QString::fromStdString(
...
void MainWindow::ConnectToPopups() {
  ...
  connect(ui->Settings, &QPushButton::clicked, this, [&]() {
    ...
    user_settings_->SetFields(ui->LoggedAs->text().toStdString(),
                              client_.GetUserId(), usd_balance, rub_balance);
    user_settings_->exec();
  });
}
...
void MainWindow::SetErrorText(const std::string &msg) {
  ui->ServerMessageInitScreen->setText(
      QString::fromStdString(s21::ResponseParser::CleanServerResponse(msg)));
}
