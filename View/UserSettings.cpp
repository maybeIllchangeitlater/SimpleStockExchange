...
UserSettings::UserSettings(QWidget *parent)
    : QDialog(parent), ui(new Ui::UserSettings) {
  ui->setupUi(this);
  connect(ui->DeleteAccount, &QPushButton::clicked, this,
          [&]() { confirm_pop_->exec(); });
  connect(confirm_pop_.get(), &PasswordConfirmationPopup::ConfirmAction, this,
  ...
  connect(namechange_pop_.get(), &ChangeNamPopup::NameChange, this,
          [&](const std::string user_name) { emit ChangeUsername(user_name); });
...
UserSettings::~UserSettings() { delete ui; }
...
void UserSettings::SetFields(const std::string &name, const std::string &id,
  ...
  ui->BalanceUSD->setText(s21::ClientDisplayMessages::usd_balance +
                          QString::fromStdString(usd_balance).remove('"'));
  ui->BalanceRUB->setText(s21::ClientDisplayMessages::rub_balance +
                          QString::fromStdString(rub_balance).remove('"'));
}
