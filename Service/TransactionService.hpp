...
class TransactionService : public TransactionServiceInterface {
 public:
  ...
  void MakeTransaction(const std::string &seller_id,
                       const std::string &buyer_id, const std::string &rate,
                       const std::string &quantity) override {
    repository_.CreateTransaction(seller_id, buyer_id, rate, quantity,
  ...
  nlohmann::json ReadTransaction(const std::string &transaction_id) override {
    return GenerateTransactionInfo(
        repository_.ReadTransaction(transaction_id)[0]);
  }

  nlohmann::json ReadAllUserSellTransactions(
  ...
  nlohmann::json ReadAllUserBuyTransactions(
      const std::string &user_id) override {
