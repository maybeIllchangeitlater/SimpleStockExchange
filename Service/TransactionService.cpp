...
nlohmann::json TransactionService::GenerateTransactionInfo(
    const pqxx::row &transaction_info) {
  ...
  transaction_json[BDNames::joined_buyer_name] =
      transaction_info[BDNames::joined_buyer_name].is_null()
  ...
  transaction_json[BDNames::joined_seller_name] =
      transaction_info[BDNames::joined_seller_name].is_null()
          ? BDNames::deleted_account
  ...
  transaction_json[BDNames::transaction_table_create_update_time] =
      transaction_info[BDNames::transaction_table_create_update_time]
          .as<std::string>();
  return transaction_json;
}

nlohmann::json TransactionService::GetQuotations(
