...
namespace s21{
    ...
    nlohmann::json BidService::UpdateBidRate(const std::string &bid_id, const std::string &rate){
        if(!ValidateRate(rate)){
            throw std::logic_error(ServerMessage::server_message.at(ServerMessage::BID_BAD_RATE));
        }
        ...
        if(!match.empty()){
            std::ptrdiff_t remaining_quantity = MakeTransactions(send_back, match, user_id, rate,
    ...
    void BidService::CancelBid(const std::string &bid_id, const std::string &user_id) {
        {
    ...
    nlohmann::json BidService::ProcessBid(const std::string &user_id, const std::string &rate,
        ...
        if(!res.empty()){
            ...
            send_back += MakeBid(user_id, rate, quantity, type); //it fails here, some param is not checked for null
        }
        return send_back;
    }


    std::ptrdiff_t BidService::MakeTransactions(nlohmann::json &send_back, pqxx::result &matched_bids,
                                      const std::string &user_id, const std::string &rate,
                                      const std::string &quantity, BidType type) {
        auto remaining_quantity = std::stoll(quantity);
        for(const auto& row : matched_bids){

            auto current_transaction = CompileCurrentTransaction(row, rate, quantity, type);

            if(type == BUYING) {
                transaction_service_.MakeTransaction(row[BDNames::bid_table_seller_id].as<std::string>(),
                                                     user_id, row[BDNames::bid_table_rate].as<std::string>(),
                                                     current_transaction[ExtraJSONKeys::buy_transaction]);
            }else{
                transaction_service_.MakeTransaction(user_id, row[BDNames::bid_table_buyer_id].as<std::string>(),
                                                     rate, current_transaction[ExtraJSONKeys::sell_transaction]);
            }

            send_back += current_transaction;

            auto transaction_amount = type == BUYING
                    ? std::stoll(std::string(current_transaction[ExtraJSONKeys::buy_transaction]))
                    : std::stoll(std::string(current_transaction[ExtraJSONKeys::sell_transaction]));
    ...
    pqxx::result BidService::MatchBids(const std::string &user_id, const std::string &rate,
                                       s21::BidService::BidType type) {
        return type == SELLING
                ? repository_.MatchBuyBids(user_id, rate)
                : repository_.MatchSellBids(user_id, rate);
    }

    nlohmann::json BidService::CompileCurrentTransaction(const pqxx::row &raw_match, const std::string &rate,
                                                         const std::string &quantity, s21::BidService::BidType type) {
        nlohmann::json current_transaction;

        auto buyer_quant = type == BUYING
                           ? std::stoll(quantity)
    ...
    void BidService::AlterAffectedBids(const pqxx::row &raw_match, std::ptrdiff_t transaction_amount) {
        auto second_party_remaining_quantity =
                raw_match[BDNames::bid_table_quantity].as<std::ptrdiff_t>() - transaction_amount;

        if (second_party_remaining_quantity <= 0) {
            repository_.DeleteBid(raw_match[BDNames::bid_table_id].as<std::string>());
        } else {
            repository_.UpdateBidQuantity(raw_match[BDNames::bid_table_id].as<std::string>(),
                                          std::to_string(second_party_remaining_quantity),
                                          Timestamper::GetTimestamp());
        }
    }

    nlohmann::json BidService::MakeBid(const std::string &user_id, const std::string &rate,
                                       const std::string &quantity, s21::BidService::BidType type) {
        nlohmann::json bid;
