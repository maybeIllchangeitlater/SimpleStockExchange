...
namespace s21{
    ...
    bool ResponseParser::CheckIfTransactionsWereMade(const std::string &msg) noexcept {
    ...
    std::string ResponseParser::CleanBidCreatedResponse(std::string &&msg) {
        ...
        msg.
                erase(std::remove_if(msg.begin(), msg.end(), [](char c) { return c == '\"' || c == ':'; }),
                      msg.end());
    ...
    std::pair<std::string, std::string> ResponseParser::ExtractCleanBalance(std::string &&msg) {
